...
for i in range(EPOCHS):
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf.split(train_feats)):
        ...
        checkpoint = tf.keras.callbacks.ModelCheckpoint(
            f'../nn_models/best_model_epoch{i + 1}_fold{fold + 1}.h5',           
            monitor='val_rmse',       
            verbose=1,               
            save_best_only=True,        
            mode='min'                
        ...
        best_model = tf.keras.models.load_model(f'../nn_models/best_model_epoch{i + 1}_fold{fold + 1}.h5', custom_objects={'RMSE': RMSE})
        ...
        test_predict = best_model.predict(X_test).squeeze(1)
